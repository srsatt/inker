...
describe('DataSourcesService', () => {
  ...
  beforeEach(() => {
    ...
    service = new DataSourcesService(mockPrisma as any, {} as any);
  });
  ...
  describe('maskSensitiveHeaders()', () => {
    const mask = (h: Record<string, string> | null) =>
      (service as any).maskSensitiveHeaders(h);

    it('should return null for null input', () => {
      expect(mask(null)).toBeNull();
    });

    it('should mask Authorization header preserving first 4 chars', () => {
      const result = mask({ Authorization: 'Bearer my-secret-token' });
      expect(result!.Authorization).toBe('Bear••••••••');
    });

    it('should mask x-api-key header', () => {
      const result = mask({ 'X-Api-Key': 'abcdef123456' });
      expect(result!['X-Api-Key']).toBe('abcd••••••••');
    });

    it('should NOT mask non-sensitive headers', () => {
      const result = mask({ 'Content-Type': 'application/json' });
      expect(result!['Content-Type']).toBe('application/json');
    });

    it('should mask short values entirely', () => {
      const result = mask({ Authorization: 'abc' });
  ...
  describe('unmaskedHeaders()', () => {
    const unmask = (
      submitted: Record<string, string> | null,
      existing: Record<string, string> | null,
    ) => (service as any).unmaskedHeaders(submitted, existing);

    it('should return null when submitted is null', () => {
      expect(unmask(null, { Authorization: 'real' })).toBeNull();
    });

    it('should return submitted as-is when existing is null', () => {
      const submitted = { Authorization: 'new-token' };
      expect(unmask(submitted, null)).toEqual(submitted);
    });

    it('should restore masked values from existing DB record', () => {
      const submitted = { Authorization: 'Bear••••••••' };
      const existing = { Authorization: 'Bearer my-secret-token' };
      const result = unmask(submitted, existing);
      expect(result!.Authorization).toBe('Bearer my-secret-token');
    });

    it('should keep user-changed values (no mask chars)', () => {
      const submitted = { Authorization: 'Bearer new-token' };
      const existing = { Authorization: 'Bearer old-token' };
      const result = unmask(submitted, existing);
      expect(result!.Authorization).toBe('Bearer new-token');
    });

    it('should handle mix of masked and unmasked headers', () => {
      const submitted = {
        Authorization: 'Bear••••••••',
      ...
      expect(result!['Content-Type']).toBe('application/json');
      expect(result!['X-Api-Key']).toBe('abcdef789');
    });

    it('should handle new headers not present in existing', () => {
      const submitted = { Authorization: 'Bear••••••••', 'X-New': 'value' };
      const existing = { Authorization: 'Bearer secret' };
      const result = unmask(submitted, existing);
  ...
  // ─── update() header safety ───────────────────────────────────────────

  describe('update() header safety', () => {
    it('should never save masked values to database', async () => {
      const realHeaders = { Authorization: 'Bearer real-secret-token-12345' };
      mockPrisma.dataSource.findUnique.mockResolvedValue({
        id: 1,
        name: 'Test',
        headers: realHeaders,
      });
      mockPrisma.dataSource.update.mockResolvedValue({ id: 1 });
      ...
      // Verify the Prisma update was called with real (unmasked) header values
      const updateCall = mockPrisma.dataSource.update.calls[0];
      const savedHeaders = updateCall[0].data.headers;
      expect(savedHeaders).toEqual(realHeaders);
      expect(JSON.stringify(savedHeaders)).not.toContain('••••••••');
    });

    it('should save new header values when user changes them', async () => {
      mockPrisma.dataSource.findUnique.mockResolvedValue({
        id: 1,
        name: 'Test',
  ...
  describe('response header masking', () => {
    const sensitiveHeaders = { Authorization: 'Bearer super-secret-token-12345' };

    it('create() should mask headers in response', async () => {
      mockPrisma.dataSource.create.mockResolvedValue({
        id: 1, name: 'New', headers: sensitiveHeaders, type: 'json', url: 'https://example.com', method: 'GET',
      });
      mockPrisma.dataSource.update.mockResolvedValue({
        id: 1, name: 'New', headers: sensitiveHeaders,
      });

      // Mock fetchDataFromSource to avoid real HTTP calls
      (service as any).fetchDataFromSource = createMock().mockResolvedValue({ ok: true });

      const result = await service.create({ name: 'New', type: 'json', url: 'https://example.com' } as any);
    ...
    it('update() should mask headers in response', async () => {
      mockPrisma.dataSource.findUnique.mockResolvedValue({ id: 1, headers: sensitiveHeaders });
